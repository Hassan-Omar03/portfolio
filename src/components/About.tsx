...
import {
  Server, Database, Layout, Globe, Shield, Cpu,
...
const services = [
  ...
  },
  {
    ...
    items: ['MongoDB & Mongoose ODM', 'Schema design & optimization', 'CRUD operations & aggregation'],
  },
  {
...
export default function About() {
  ...
  return (
    <section id="about" className="py-24 bg-[#080f1e] text-white relative overflow-hidden" ref={sectionRef}>
      {/* Background */}
      <div className="absolute inset-0 grid-pattern opacity-40" />
      <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-blue-600/6 rounded-full filter blur-[120px]" />
      ...
      <div className="container mx-auto px-6 relative z-10">
        ...
        <div className="grid lg:grid-cols-2 gap-16 items-center max-w-6xl mx-auto mb-24">
          ...
          <div className={`relative group transition-all duration-700 delay-100 ${visible ? 'opacity-100 translate-x-0' : 'opacity-0 -translate-x-12'}`}>
            <div className="absolute -inset-1 bg-gradient-to-r from-blue-600 to-teal-500 rounded-3xl blur-2xl opacity-20 group-hover:opacity-35 transition-opacity duration-500" />
            <div className="relative aspect-[4/5] rounded-3xl overflow-hidden border border-white/5">
              ...
              <div className="absolute bottom-6 left-6 right-6">
                <div className="glass-card rounded-2xl px-5 py-4 border border-white/10">
                  <p className="font-bold text-lg text-white">Mohammad Hassan</p>
                  <p className="text-blue-400 text-sm mt-0.5">Full Stack MERN Developer · Punjab, Pakistan</p>
                </div>
              </div>
            </div>

            {/* Floating badges */}
            <div className="absolute -top-4 -right-4 glass-card border border-white/10 rounded-2xl px-4 py-3 shadow-xl animate-float">
            ...
            <div className="absolute -bottom-4 -left-4 glass-card border border-white/10 rounded-2xl px-4 py-3 shadow-xl animate-float animation-delay-2000">
              <div className="flex items-center gap-2">
                <span className="text-xl">⚡</span>
                <div>
                  <p className="text-white font-bold text-sm leading-none">2+ Years</p>
          ...
          {/* Bio content */}
          <div className={`space-y-7 transition-all duration-700 delay-200 ${visible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-12'}`}>
            <div>
              ...
              <p className="text-slate-400 leading-relaxed text-base mb-4">
                ...
                experience building responsive, scalable, and production-optimized websites for
              ...
              <p className="text-slate-400 leading-relaxed text-base">
                ...
                where I completed a 1-year intensive MERN training. Since then I've worked as an intern at
                <span className="text-teal-400 font-medium"> Digital Brains</span> and have been freelancing
                full-time since <span className="text-white font-medium">January 2024</span> — delivering real
                products for real clients. I actively use AI tools like ChatGPT and prompt engineering
                to write cleaner code, debug faster, and ship sooner.
              </p>
            </div>
            ...
            <div className="flex gap-6 pt-2">
              {[['10+', 'Projects'], ['5+', 'Countries'], ['100%', 'Satisfaction']].map(([val, lbl]) => (
                <div key={lbl} className="text-center">
                  <p className="text-2xl font-bold gradient-text-animated">{val}</p>
        ...
        <div className={`transition-all duration-700 delay-300 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`}>
          ...
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5 max-w-6xl mx-auto">
            {services.map((svc, i) => (
              <div
                key={svc.title}
                className={`group glass-card rounded-2xl p-6 hover:border-white/15 hover:bg-white/[0.06] transition-all duration-400 hover:shadow-xl ${svc.glow} hover:-translate-y-1 cursor-default ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}
                style={{ transitionDelay: `${400 + i * 80}ms`, transition: 'all 0.4s ease' }}
              >
                {/* Icon */}
                <div className={`inline-flex p-3 rounded-xl bg-gradient-to-br ${svc.color} text-white mb-4 shadow-lg group-hover:scale-110 transition-transform duration-300`}>
                  {svc.icon}
                </div>
                <h4 className="text-white font-bold text-base mb-3">{svc.title}</h4>
        ...
        {/* ── TOOLS ── */}
        <div className={`mt-20 text-center transition-all duration-700 delay-500 ${visible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
          <p className="section-label mb-6">MY TOOLBOX</p>
          <div className="flex flex-wrap justify-center gap-3 max-w-3xl mx-auto">
            {tools.map((t, i) => (
              <span
                key={t}
                className="px-4 py-2 glass-card rounded-full text-sm text-slate-300 border border-white/5 hover:border-blue-400/40 hover:text-blue-300 hover:bg-blue-500/5 hover:scale-105 transition-all duration-300 cursor-default"
